-- Corrected activation for a prepared version 1 PostgreSQL deployment.
--
-- The published 0.2.0 activation artifact is immutable, but it cannot complete:
-- it validates the version-2 marker constraint while the marker row is still
-- version 1, and it names only the version-2 delivery foreign key when removing
-- the version-1 constraint. Run this replacement after the unchanged prepare
-- artifact and the operator-owned tenant backfill.
...
ALTER TABLE dovecote_deliveries
    DROP CONSTRAINT IF EXISTS dovecote_deliveries_event_row_id_fkey;
ALTER TABLE dovecote_deliveries
