-- Dovecote schema version 2, tenant-aware baseline.
--
-- This artifact is for a clean tenant-aware installation.  An existing schema
-- version 1 must use the documented prepare/backfill/activate route instead;
-- this SQL deliberately does not guess a tenant for existing rows.
...
CREATE TABLE dovecote_deliveries (
    event_row_id BIGINT PRIMARY KEY,
    tenant_id VARCHAR(255) COLLATE "C" NOT NULL,
    ...
    CONSTRAINT dovecote_deliveries_event_fk FOREIGN KEY (tenant_id, event_row_id)
        REFERENCES dovecote_events (tenant_id, row_id) ON DELETE RESTRICT
);
