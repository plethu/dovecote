-- Schema version 2: clean tenant-aware MySQL/MariaDB installation.
-- Version 1 remains immutable; existing deployments use the explicit
-- prepare/backfill/activate route documented by the adapter.
...
CREATE TABLE dovecote_schema (
    ...
    CONSTRAINT dovecote_schema_minimum_nonnegative CHECK (minimum_crate_major >= 0 AND minimum_crate_minor >= 0 AND minimum_crate_patch >= 0)
) ENGINE = InnoDB;
INSERT INTO dovecote_schema VALUES (2, 0, 2, 0, FALSE);
...
CREATE TABLE dovecote_events (
    ...
    -- The length prefixes make the identity encoding injective without
    -- relying on a forbidden separator surviving a direct SQL write.  The
    -- source and event-id contract still limits the suffix to 2,048 bytes:
    -- 3 + 255 + 4 + 2,048 = 2,310 bytes.
    identity_key VARBINARY(2310) GENERATED ALWAYS AS (
        CONCAT(
            LPAD(OCTET_LENGTH(tenant_id), 3, '0'),
            tenant_id,
            LPAD(OCTET_LENGTH(source), 4, '0'),
            source,
            event_id
        )
    ) STORED,
    CONSTRAINT dovecote_events_specversion CHECK (specversion = _binary '1.0'),
    CONSTRAINT dovecote_events_tenant_size CHECK (OCTET_LENGTH(tenant_id) <= 255),
    CONSTRAINT dovecote_events_tenant_nonempty CHECK (OCTET_LENGTH(tenant_id) > 0),
    ...
    CONSTRAINT dovecote_events_tenant_row_unique UNIQUE (tenant_id, row_id),
    UNIQUE KEY dovecote_events_tenant_source_event_id (identity_key)
) ENGINE = InnoDB;
...
CREATE TABLE dovecote_deliveries (
    ...
    claim_token BINARY(16), claimed_by VARBINARY(255), claim_expires_at DATETIME(6),
    last_failure_code VARBINARY(128), last_failure_detail VARBINARY(2048),
    delivered_at DATETIME(6), quarantined_at DATETIME(6), quarantine_reason VARBINARY(2048),
    ...
    CONSTRAINT dovecote_deliveries_tenant_size CHECK (OCTET_LENGTH(tenant_id) <= 255),
    CONSTRAINT dovecote_deliveries_tenant_nonempty CHECK (OCTET_LENGTH(tenant_id) > 0),
    ...
    CONSTRAINT dovecote_deliveries_event_fk FOREIGN KEY (tenant_id, event_row_id) REFERENCES dovecote_events (tenant_id, row_id) ON DELETE RESTRICT,
